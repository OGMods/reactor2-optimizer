 ...
 * shares. Port of `solver/constants.py` (these values were split out of `rules.py`;
 * the adjacency geometry that stayed behind there lives in `context.ts` on this
 * side). See `docs/game-logic.md` for the authoritative
 * rules.
 */
 ...
 * descending Y. Equivalent to Python's `spatial_key` = `(x, -y)`.
 */
