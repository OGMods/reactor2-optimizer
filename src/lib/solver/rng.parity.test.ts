/**
 * Pins `rng.ts` to the same stream `py_solver/solver/rng.py` produces.
 *
 * docs/PARITY.md calls this the easiest contract in the codebase to break without
 * noticing, and it is the foundation the fixture harness stands on: if the two
 * RNGs drift, every golden fixture is invalidated at once and the failures show
 * up somewhere far less obvious than here.
 *
 * The values below are the same goldens pinned in `tests/test_rng.py`. If one
 * of these fails, fix the implementation that drifted -- do NOT re-record.
 */
...
describe("rng python parity", () => {
  ...
  it("shuffles to the same permutation Python produces", () => {
    const a = Array.from({ length: 20 }, (_, i) => i);
    new Rng(77).shuffle(a);
    // Captured from solver/rng.py: Rng(77).shuffle(list(range(20))).
    expect(a).toEqual([
      13, 16, 7, 6, 11, 2, 4, 3, 5, 9, 15, 18, 10, 14, 17, 1, 19, 12, 0, 8,
    ]);
  });

  it("picks the same choice Python does", () => {
