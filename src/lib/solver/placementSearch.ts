...
/**
 * Placement search for a single island. Port of `solver/placement_search.py`:
 *
 ...
 * bit for bit. That is what lets a run be replayed against the Python
 * reference (`solver/placement_search.py`). The solve pipeline itself never
 * sets it — wall time is the right budget in production, where per-step cost
 * varies by island.
 */
...
function targetCompositions(
  ...
  // Stable sort keeps enumeration order among equal-scoring compositions,
...
async function arrangeComposition(
  ...
  // Python enumerates `sorted(current)` — the (x, y) keys in plain ascending
  // order, which is NOT the tile index order (ascending X, DESCENDING Y).
 ...
 *
 * Tiles are visited in ascending index order, which is the spatial order the
 * Python reference sorts for explicitly — see `context.ts`.
 */
 ...
 */
/**
 * Deterministic replay of `solveIsland`'s reproducible stages, for the
 * Python↔TypeScript parity harness. Not used in production.
 *
 ...
 * replay them. `parity/export_fixtures.py` on the Python side omits exactly the
 * same stages — the two must stay in lockstep or every fixture breaks.
 */
export async function replayIslandDeterministic(
  ...
  // Matches UNREACHABLE_DEADLINE_S in parity/export_fixtures.py (1e6 seconds).
