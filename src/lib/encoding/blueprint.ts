...
import type { PlacedBuilding, Tile, TileType } from "../types";
 ...
 *
 * This is the only layout format in the app: island templates
 * (`lib/data/maps.ts`), saved layouts, share codes, and the Python parity
 * fixtures all use it. Its peer is `py_solver/blueprint.py`, and the byte
 * tables below **must** match it — a code written by one tree is read by the
 * other, so a renumbered building decodes as a *different* building rather
 * than failing. `py_solver/tests/test_blueprint.py` pins the tables against
 * this file.
 */
